...
router.get("/dashboard", async (req, res) => {
  try {
    ...
    console.log("📥 Petición recibida en /dashboard de:", usuario);
    ...
    if (usuario.rol === "docente") {
      ...
      return res.json({
        ...
        // ===== USUARIO NORMAL (antes "alumno") =====
    if (usuario.rol === "alumno" || usuario.rol === "usuario") {
      const progreso = [
        { materia: "Programación", avance: 60 },
        { materia: "IA Aplicada", avance: 40 },
      ];

      return res.json({
        totalMaterias: progreso.length,
        progreso,
        retroalimentacion: "Sigue practicando tus quizzes, vas muy bien 🚀",
        ultimaActualizacion: new Date().toISOString(),
      });
    }
    ...
    console.error("❌ Error en /protected/dashboard:", error);
